...
module Scope_Quotation_Macro : sig
  ...
  let run state ~toplevel_macros p : Scoped.program =
    ...
    let rec compile_program omacros state { Ast.Structured.macros; kinds; types; type_abbrevs; body } =
      ...
      toplevel_macros,
      { Scoped.types; kinds; type_abbrevs; body; symbols }
...
module Flatten : sig
  ...
  let rec compile_block kinds types type_abbrevs modes clauses chr subst = function
    ...
    | Scoped.Shorten(shorthands, { kinds = k; types = t; type_abbrevs = ta; body; symbols = _ }) :: rest ->
  ...
  | Scoped.Namespace (extra, { kinds = k; types = t; type_abbrevs = ta; body; symbols = s }) :: rest ->
  ...
  | Scoped.Constraints (ch, { kinds = k; types = t; type_abbrevs = ta; body }) :: rest ->
      ...
      compile_block kinds types type_abbrevs modes clauses chr subst rest
  | Scoped.Accumulated { kinds=k; types = t; type_abbrevs = ta; body; symbols = _ } :: rest ->
      let kinds = merge_kinds (apply_subst_kinds subst k) kinds in
      let types = merge_types (apply_subst_types subst t) types in
      let type_abbrevs = merge_type_abbrevs type_abbrevs (apply_subst_type_abbrevs subst ta) in
      (* let modes = merge_modes (apply_subst_modes subst m) modes in *)
  ...
  let compile_body { Scoped.kinds; types; type_abbrevs; symbols; body } =
    compile_block kinds types type_abbrevs () [] [] empty_subst body
...
let pp_program (pp : pp_ctx:pp_ctx -> depth:int -> _) fmt (compiler_state, { Assembled.clauses; symbols; signature={types} }) =
  ...
  F.Map.iter (fun k v -> Format.fprintf fmt "@[<h>%%%a : %a@]@;" F.pp k TypeAssignment.pretty_overloaded_skema v) types;
